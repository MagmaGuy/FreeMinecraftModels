...
public class StorageDoublePropConfig extends PropScriptLuaConfigFields {
    ...
    static final String STORAGE_SCRIPT_TEMPLATE = """
            ...
            local function can_bypass_region_protection(player)
                if not player then return false end
                if player.has_permission and player:has_permission("freeminecraftmodels.bypassregionprotection") then
                    return true
                end
                return player.is_op == true
            end
            ...
            return {
                ...
                on_right_click = function(context)
                    ...
                    if protect_loc and not can_bypass_region_protection(player)
                            and (em.location.is_protected(protect_loc) or em.location.is_in_dungeon(protect_loc)) then
    ...
    static final String LEGACY_STORAGE_SCRIPT_TEMPLATE = """
            -- Premade storage script for props that act as chests/drawers/closets.
            -- Configure via the variables below.
            --
            -- INVENTORY_TITLE: the name shown at the top of the chest GUI (supports color codes)
    ...
    public List<String> getLegacySources() {
        return List.of(LEGACY_STORAGE_SCRIPT_TEMPLATE.formatted(
                "&8Storage", 6,
                "\"open\"", "\"close\"",
                "\"BLOCK_CHEST_OPEN\"", "\"BLOCK_CHEST_CLOSE\""));
    }
