...
//    }
    ...
    protected String processString(String path, String value, String pluginDefault, boolean forceWriteDefault) {
        ...
        try {
            ...
            Developer.warn("File " + filename + " has an incorrect entry for " + path);
            Developer.warn("Entry: " + value);
        }
        return value;
    }
        ...
        try {
            ...
            stringList.forEach(string -> {
                try {
                    ...
                    Developer.warn(filename + " : " + "Value " + string + " is not a valid for " + path + " ! This may be due to your server version, or due to an invalid value!");
                }
            });
            return newList;
        } catch (
    ...
    public <T extends Enum<T>> T processEnum(String path, T value, T pluginDefault, Class<T> enumClass, boolean forceWriteDefault) {
        ...
        try {
            ...
            Developer.warn("File " + filename + " has an incorrect entry for " + path);
            Developer.warn("Entry: " + fileConfiguration.getString(path));
            value = null;
        }
        if (value == null)
